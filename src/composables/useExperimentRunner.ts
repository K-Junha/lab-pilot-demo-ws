...
function computeFiringTarget(segments: { rampRate: number | null; temp: number | null; holdMin: number | null }[], elapsed: number): number {
  ...
  for (const seg of segments) {
    ...
    const rampTime = Math.abs(target - currentTemp) / rate
...
const currentRun = ref<ExperimentRun | null>(null)
...
export function useExperimentRunner() {
  ...
  function createRun(workflow: Workflow): ExperimentRun {
    ...
    // Auto-complete composition-design related steps (weighing)
    // They are already done during workflow design
    const compositionSteps = steps.filter(s => s.type === 'weighing')
    compositionSteps.forEach(s => {
      s.status = 'completed'
